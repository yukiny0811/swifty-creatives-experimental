...
//
//  Original code from https://github.com/jscalo/tempi-fft (CC0 1.0 Universal)
...
public class TempiFFT : NSObject {
    ...
    private(set) public var numberOfBands: Int = 0
...
//        print("halfSize", halfSize)
    ...
    private func magsInFreqRange(_ lowFreq: Float, _ highFreq: Float) -> [Float] {
...
//        print(lowIndex, highIndex, #line)
    ...
    public func frequencyAtBand(_ inBand: Int) -> Float {
        assert(hasPerformedFFT, "*** Perform the FFT first.")
        assert(bandMagnitudes != nil, "*** Call calculateLinearBands() or calculateLogarithmicBands() first")
